/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { UserLoginHistoryWhereInput } from "./UserLoginHistoryWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { UserLoginHistoryOrderByInput } from "./UserLoginHistoryOrderByInput";

@ArgsType()
class UserLoginHistoryFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => UserLoginHistoryWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => UserLoginHistoryWhereInput, { nullable: true })
  @Type(() => UserLoginHistoryWhereInput)
  where?: UserLoginHistoryWhereInput;

  @ApiProperty({
    required: false,
    type: [UserLoginHistoryOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [UserLoginHistoryOrderByInput], { nullable: true })
  @Type(() => UserLoginHistoryOrderByInput)
  orderBy?: Array<UserLoginHistoryOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { UserLoginHistoryFindManyArgs as UserLoginHistoryFindManyArgs };
